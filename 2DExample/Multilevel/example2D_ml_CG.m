function example2D_ml_CG(r,r_deim,mode,errormode,cg_mode,linesearch_mode,pod_mode,mminvec,nvec)
...
% -------------------------------------------------------------------------
%
% The results will be saved in example2D_results_cg.mat
%
% =========================================================================
% notes
% Übergebe altes ergebniss als neuen Startwert
% Projektion von altem auf neues Netz??
% berechnung netz in diese funktion da wir m brauchen um startwert
% festzulegen-->eigentlich nicht, da nur abhängig von n, numcontrols
% 
%
%
% Parameters
...
if(mode == 0 || mode == 1)
   ...
   usnap(:,1) = 0.5*ones(n,1);  % controls for generation of snapshots
    ...
    [yopt_fem,uiopt_fem,popt_fem,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay] = multilevel_cg_fem(mmin, n, mode, cg_mode,linesearch_mode,ustart , lambda, c, deltat,TOL1,TOL2,s0); 
    ...
    % Interpolation on the new time-grid
           ...
           [yopt_fem,uiopt_fem,popt_fem,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay,y0,fxyt,K,tri] =multilevel_cg_fem(mmin, n,  mode, cg_mode,linesearch_mode,ustart, lambda, c, deltat,TOL1,TOL2,s0); 
    %    end
...
if(mode == 0 || mode == 2)
    ...
    if pod_mode==0 || pod_mode==1
        ...
        [yopt_pod,uiopt_pod,popt_pod,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay,y0,fxyt,K,tri] = multilevel_cg_pod(mmin, n, r, mode, cg_mode,linesearch_mode,pod_mode, ustart ,  lambda, c, deltat,TOL1,TOL2,s0,usnap); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if pod_mode==0 || pod_mode==2
            ...
            [yopt_pod_deim,uiopt_pod_deim,popt_pod_deim,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay,y0,fxyt,K,tri] = multilevel_cg_pod_deim(mmin, n, r,r_deim, mode, cg_mode,linesearch_mode,pod_mode, ustart ,  lambda, c, deltat,TOL1,TOL2,s0,usnap); 
