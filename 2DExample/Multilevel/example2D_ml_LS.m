function example2D_ml_LS(r,r_deim,mode,errormode,linesearch_mode,pod_mode,mminvec,nvec)
...
if(mode == 0 || mode == 1)
   ...
   usnap(:,1) = 0.5*ones(n,1);  % controls for generation of snapshots
    ...
    [yopt_fem,uiopt_fem,popt_fem,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay] = multilevel_sd_fem(mmin, n, mode, linesearch_mode, ustart ,lambda, c, deltat,TOL1,TOL2,s0); 
    ...
    % Interpolation on the new time-grid
           ...
           [yopt_fem,uiopt_fem,popt_fem,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay,y0,fxyt,K,tri] =multilevel_sd_fem(mmin, n, mode,linesearch_mode, ustart, lambda, c, deltat,TOL1,TOL2,s0); 
    %    end
...
if(mode == 0 || mode == 2)
    ...
    if pod_mode==0 || pod_mode==1
        ...
        [yopt_pod,uiopt_pod,popt_pod,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay,y0,fxyt,K,tri] = multilevel_sd_pod(mmin, n, r,mode,linesearch_mode,pod_mode, ustart , lambda, c, deltat,TOL1,TOL2,s0,usnap); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if pod_mode==0 || pod_mode==2
            ...
            [yopt_pod_deim,uiopt_pod_deim,popt_pod_deim,m,x1,x2,au,M,V1,V2,V3,V4,v,yq,Mt,ay,y0,fxyt,K,tri] = multilevel_sd_pod_deim(mmin,n, r, r_deim,mode, linesearch_mode,pod_mode, ustart ,  lambda, c, deltat,TOL1,TOL2,s0,usnap); 
