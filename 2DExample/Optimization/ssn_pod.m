function [yopt,uopt,popt] = ssn_pod(n,r,xy,tri,deltat,lambda,c,dxyt,ay,au,ua,ub,ustart,ystart,pstart,M,K,Mt,y0,yq,POD)
...
while (diff > TOL && counter < countermax)
    ...
    for k = 1:(n-1)
        ...
        if k == 1
            ...
            for ip = 1:r
                for iq = 1:r
                    Mck(ip,iq) = yk(:,k)'*Mpod4(:,:,ip,iq)*yk(:,k);
                end
            end
        else
            Mck = Mak;
        ...
        for ip = 1:r
            for iq = 1:r
                Mak(ip,iq) = yk(:,k+1)'*Mpod4(:,:,ip,iq)*yk(:,k+1);
            end
        end
