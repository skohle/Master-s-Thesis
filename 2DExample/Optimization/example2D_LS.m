function example2D_LS(mmin,n,r,r_deim,mode,errormode,linesearch_mode,pod_mode)
...
% -------------------------------------------------------------------------
% 
% Used algorithms:  Linesearch method, finite
...
% uipoddeim         - optimal POD-DEIM controls u_1, ..., u_4
% ypoddeim          - optimal POD-DEIM state
% ppoddeim          - corresponding POD-DEIM adjoint state
...
if(mode == 0 || mode == 1)
    ...
    [yopt_fem,uiopt_fem,popt_fem] = ls_fem(m,deltat,lambda,c,fxyt,ay,au,ua,ub,ustart,M,K,y0,yq,V1,V2,V3,V4,v,Mt,linesearch_mode,TOL1,TOL2,s0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if pod_mode==0 || pod_mode==1
        ...
        [yopt_pod,uiopt_pod,popt_pod] = ls_pod(n,r, x1x2,tri,deltat,lambda,c,fxyt,ay,au,ua,ub,ustart,Mpod,Kpod,Mt,M,y0pod,yqpod,POD,linesearch_mode,TOL1,TOL2,s0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if pod_mode==0 || pod_mode==2
            ...
            TOL2 = 5*1e-5;
            timestart_podopt = cputime;
            [yopt_pod_deim,uiopt_pod_deim,popt_pod_deim] = ls_pod_deim(n,r,r_deim,deltat,lambda,c,fxyt,ay,au,ua,ub,ustart,Mpod,Kpod,Mt,M,y0pod,yqpod,POD,U,P,linesearch_mode,TOL1,TOL2,s0);
            time_podopt = cputime - timestart_podopt;
