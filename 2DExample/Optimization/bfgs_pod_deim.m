function [yopt_pod,uiopt_pod,popt_pod] = bfgs_pod_deim(r,r_deim, x1x2,deltat,lambda,c,fxyt,ay,au,ua,ub,ustart,Mpod,Kpod,Mt,M,y0pod,yqpod,POD,U,P,TOL1,TOL2,s0,linesearch_mode,bfgs_mode)
% =========================================================================
...
J_uk=objvalue_pod(uk, ykpod, yqpod, ay, lambda, M, Mt, POD, Mpod);
    ...
    switch linesearch_mode    
        case 1 
            ...
            error('myApp:argChk', 'Bisection is not suitable for BFGS, set linesearch_mode=3 or 4')
        case 2 
            %disp('Bisection resp. Armijo is not suitable for BFGS')
            error('myApp:argChk', 'Armijo is not suitable for BFGS, set linesearch_mode=3 or 4')
        case 3 %Wolfe-Powell
            %disp('***          ---> Wolfe-Powell');
            if counter==15
                s0=2;
