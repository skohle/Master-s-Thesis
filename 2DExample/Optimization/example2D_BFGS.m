function example2D_BFGS(mmin, n, r, r_deim, mode, errormode,linesearch_mode,pod_mode)
...
% -------------------------------------------------------------------------
% 
% Used algorithms:  BFGS method, finite
...
TOL1 = 5*1e-6;                 % Tolerance obj_value
...
switch bfgs_mode
    case 1% Identity
        disp('Restart BFGS: Bk=I');              
    case 2% keep the diagonal elements
        disp('Restart BFGS: Bk=(\)');
    case 3 %keep the tridiagonal elements
        disp('Restart BFGS: Bk=tridiag(Bk)');         
    case 4
        disp('Restart BFGS: Bk(i,i)=sum(Bk(:,i)');                
    case 5
        disp('Restart BFGS: Bk(i,i)=sum(Bk(i,:)');                           
    case 6
        disp('Restart BFGS: Bk(i,i)=max(Bk(:,i)');                
end  
disp(' ');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1) Computation by FE method
...
if(mode == 0 || mode == 1)
    ...
    disp('*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*');
    disp('    Optimization by BFGS using FEM');
    disp('*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*');
    timestart = cputime;
    TOL2 = 3.2*1e-3;
    [yopt_fem,uiopt_fem,popt_fem] = bfgs_fem(m,deltat,lambda,c,fxyt,ay,au,ua,ub,ustart,M,K,y0,yq,V1,V2,V3,V4,v,Mt,TOL1,TOL2,s0,linesearch_mode,bfgs_mode);
...
if(mode == 0 || mode == 2)
    ...
    if pod_mode==0 || pod_mode==1
        ...
        TOL2 = 3.5*1e-3;
        s0=5; 
        [yopt_pod,uiopt_pod,popt_pod] = bfgs_pod(r, x1x2,tri,deltat,lambda,c,fxyt,ay,au,ua,ub,ustart,Mpod,Kpod,Mt,M,y0pod,yqpod,POD,linesearch_mode,TOL1,TOL2,s0,bfgs_mode);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if pod_mode==0 || pod_mode==2
            ...
            s0=5; 
            timestart_podopt = cputime;
            [yopt_pod_deim,uiopt_pod_deim,popt_pod_deim] = bfgs_pod_deim(r,r_deim, x1x2,deltat,lambda,c,fxyt,ay,au,ua,ub,ustart,Mpod,Kpod,Mt,M,y0pod,yqpod,POD,U,P,TOL1,TOL2,s0,linesearch_mode,bfgs_mode);
            time_podopt = cputime - timestart_podopt;
...
if ( mode == 1 || mode == 0)
    ...
    optimality_test('FEM',m,n,t,V1,V2,V3,V4,popt_fem,uiopt)
end
...
if ( (mode == 2 || mode == 0) && (pod_mode==1 || pod_mode==0))
    ...
    optimality_test('POD',m,n,t,V1,V2,V3,V4,popt_pod,uiopt)
   
    
    % with u compute by POD and y,p by the full equation
    % optimality systems uipodi <---> ppod
    
    optimality_test('PODfull',m,n,t,V1,V2,V3,V4,pf_pod,uiopt)
...
if ( (mode == 2 || mode == 0) && (pod_mode==2 || pod_mode==0))
    ...
    optimality_test('POD-DEIM', m,n,t,V1,V2,V3,V4,popt_pod_deim,uiopt)

    
    % with u compute by POD and y,p by the full equation
    % optimality systems uipodi <---> ppod
    
    optimality_test('POD-DEIMfull',m,n,t,V1,V2,V3,V4,pf_pod_deim,uiopt)
